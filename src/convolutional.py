...
def set_convolutional(X, W, b, stride, bn_beta, bn_gamma, bn_mm, bn_mv, filtergroup=False, batchnorm=True,
                      activation=True, scope=None, reuse=False, trainable=False):
    # use the input scope or default to "conv"
    with tf.variable_scope(scope or 'conv', reuse=reuse):
        # sanity check    
        W = tf.get_variable("W", W.shape, trainable=trainable, initializer=tf.constant_initializer(W))
        b = tf.get_variable("b", b.shape, trainable=trainable, initializer=tf.constant_initializer(b))
        ...
        if batchnorm:
            h = tf.layers.batch_normalization(h, beta_initializer=tf.constant_initializer(bn_beta),
                                              ...
                                              training=trainable, trainable=False)
